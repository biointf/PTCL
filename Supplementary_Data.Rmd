---
title: "PTCL Gene Expression based stratification"
author:
- affiliation: University of Milan
  name: Luca Agnelli
- affiliation: Memorial Sloan Kettering Cancer Center, NY, USA; Wellcome Trust Sanger Institute, UK and University of Milan, IT
...
results_clust = ConsensusClusterPlus(mat_clust,maxK=8,
                                     ...
                                     distance="euclidean", 
                                     seed=123456789)
kk<- as.data.frame((results_clust[[8]]$consensusClass)) ##### 6 clusters considering the orioginla 6 different histologies
...
geneannotation1 <- getBM( attributes = c("ensembl_transcript_id", "entrezgene", "external_gene_name"), filters = "entrezgene", values = gsub("_at","",kk_tab2$gene), mart = ensembl)
...
# write.table(tab_save, "/Volumes/GoogleDrive/My Drive/PTCL/AJH/table_differentially_expressed_gene.txt",sep="\t", quote=F, row.names = F, col.names = T)
...
# write.table((tab23), "/Volumes/GoogleDrive/My Drive/PTCL/AJH/table_differentially_expressed_gene_true_false_per mutation.txt",sep="\t", quote=F, row.names = F, col.names = T)
...
table(dd$LOOCV.predicted, dd$true.class)
  ...
  # ALCL-ALKneg    0           4        2
  # PTCL-NOS       7           2        2
table(dd$LOOCV.predicted, dd$RHOA)
  #             0 1
  # AITL        9 4
  # ALCL-ALKneg 6 0
  # PTCL-NOS    9 2
table(dd$LOOCV.predicted, dd$TET2)
  #             0 1
  # AITL        8 5
  # ALCL-ALKneg 6 0
  # PTCL-NOS    9 2
table(dd$LOOCV.predicted, dd$IDH2)
  #            0  1
  # AITL        11  2
  # ALCL-ALKneg  6  0
...
pheatmap(mat3, annotation_col = mylabel.nocol, annotation_colors = list(clusters = c(cluster1= mycol_plus[1], cluster2 = mycol_plus[2], cluster3 = mycol_plus[3], cluster4 = mycol_plus[4], cluster5 = mycol_plus[5]), Histology = c(AITL = "black", PTCL.nos = "orange", ALCL.neg= "yellow"), IDH2 = c(MUT=mycol[1], "NA"=mycol[2],WT=mycol[3]), RHOA = c(MUT=mycol[1],"NA"=mycol[2],WT=mycol[3]), TET2 = c(MUT=mycol[1],"NA"=mycol[2],WT=mycol[3]), DNMT3A = c(MUT=mycol[1],"NA"=mycol[2],WT=mycol[3]) ) ,  border_color= NA, color = colorRampPalette(rev(brewer.pal(n = 5 , name = "RdYlBu")))(100), scale = "row", cluster_cols = FALSE, show_colnames= F, row_annotation =3, cellheight = 20,
         # gaps_col=c(0,rep(0,num[1]-1), 40,rep(0,num[2]-1), 1000,rep(0,num[3]-1), 40,rep(0,num[4]-1), 
         #            40,rep(0,num[5]-1)))
         gaps_col = num)
# dev.off()
```
...
#cibersort.percentages<- read.delim("./Rmd.files/cibersort.percentages.txt", sep="\t", stringsAsFactors = F, header = T)
...
tmodPanelPlot(res.l2, pie=pie, text.cex=0.5) ##### zero = grey, blue down in the first factor and red up in the first
